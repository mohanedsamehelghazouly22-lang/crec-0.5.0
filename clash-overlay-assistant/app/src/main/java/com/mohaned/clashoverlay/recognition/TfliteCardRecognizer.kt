...
/** Real on-device classifier. No model means no guesses: never fabricate card identities. */
class TfliteCardRecognizer(context: Context) : AutoCloseable {
    ...
    init {
        ...
        labels = runCatching {
            context.assets.open("models/labels.txt")
                .bufferedReader()
                .readLines()
                .map(String::trim)
    ...
    fun classify(bitmap: Bitmap): Pair<String, Float>? {
        ...
        return try {
            ...
            val probs = readOutput(tflite, outputTensor, classes, inputBuffer)
            val best = probs.indices.maxByOrNull { probs[it] } ?: return null
            val confidence = probs[best].coerceIn(0f, 1f)
            labels.getOrNull(best)?.let { it to confidence }
        } finally {
            resized.recycle()
        }
    }
    ...
    private fun readOutput(
        ...
        classes: Int,
        input: ByteBuffer
    ): FloatArray = when (tensor.dataType()) {
        DataType.FLOAT32 -> {
            val out = Array(1) { FloatArray(classes) }
            tflite.runForMultipleInputsOutputs(arrayOf(input), mapOf(0 to out))
        ...
        DataType.UINT8, DataType.INT8 -> {
            ...
            normalizeScores(out[0].map {
                val raw = if (tensor.dataType() == DataType.UINT8) it.toInt() and 0xFF else it.toInt()
    ...
    private fun normalizeScores(values: FloatArray): FloatArray {
        if (values.all { it in 0f..1f }) return values
        val maxValue = values.maxOrNull() ?: return values
        val exps = FloatArray(values.size)
        var sum = 0.0
        values.forEachIndexed { i, v ->
            val e = kotlin.math.exp((v - maxValue).toDouble()).toFloat()
            exps[i] = e
            sum += e
        }
        if (sum <= 0.0) return values
        for (i in exps.indices) exps[i] = (exps[i] / sum).coerceIn(0f, 1f)
        return exps
    }
    ...
    private fun makeInput(bitmap: Bitmap, tensor: org.tensorflow.lite.Tensor): ByteBuffer {
        ...
        val buffer = ByteBuffer.allocateDirect(bitmap.width * bitmap.height * 3 * bytes)
            .order(ByteOrder.nativeOrder())
        val pixels = IntArray(bitmap.width * bitmap.height)
        bitmap.getPixels(pixels, 0, bitmap.width, 0, 0, bitmap.width, bitmap.height)
        val q = tensor.quantizationParams()
        if (type != DataType.FLOAT32 && q.scale <= 0f) error("Invalid model quantization scale")
        ...
        pixels.forEach { pixel ->
            ...
            channels.forEach { channel ->
                if (type == DataType.FLOAT32) {
                    ...
                    val quantized = (channel / 255f / q.scale + q.zeroPoint).roundToInt()
